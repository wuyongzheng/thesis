...
\put(120,15){\deffigp{fp:gp-gimp}}
\put(145,180){\vector(1,-3){15}}
\put(135,187){\deffigp{fp:gp-gegl}}
\put(220,205){\vector(1,-2){15}}
\put(205,207){\deffigp{fp:gp-gtk}}
\put(467,170){\vector(1,-1){25}}
\put(450,172){\deffigp{fp:gp-net}}
\put(640,15){\vector(1,0){50}}
\put(620,10){\deffigp{fp:gp-zlib}}
\end{overpic}
...
e.g. {\tt gimp} (\usefigp{fp:gp-gimp}) uses
the {\tt [gegl]} (\usefigp{fp:gp-gegl}) image processing modules,
which in turn uses the {\tt [gtk]} (\usefigp{fp:gp-gtk}) windowing
module.
...
(\usefigp{fp:gp-net}) is only used by the {\tt [glib]} library directly
and the compression module (\usefigp{fp:gp-zlib}) is only used by
