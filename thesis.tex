\documentclass[11pt,a4paper,openany]{book}

\usepackage[hyphens,obeyspaces,spaces]{url}
\usepackage{amsmath}
\usepackage{subfigure}
\usepackage{graphicx}
\usepackage{algorithm2e}
\usepackage[bookmarksnumbered]{hyperref}
\usepackage{color}
\usepackage{multirow}
%\usepackage{appendix}
%\usepackage[section]{placeins}
\usepackage[a4paper,includeheadfoot,hmargin=1.2in,vmargin=1.0in]{geometry}
\usepackage[onehalfspacing]{setspace}
\usepackage{rotating}
\usepackage[permil]{overpic} % overpic v0.53 has a problem shifting X. I use v0.52 instead.
\usepackage{tikz}


\newcounter{figp}
\newcommand\figpcir[1]{\tikz[baseline=(char.base)]{
            \node[shape=circle,draw,inner sep=1pt](char){#1};}}
\newcommand{\usefigp}[1]{\figpcir{\ref{#1}}}
\newcommand{\deffigp}[1]{\refstepcounter{figp}\label{#1}\usefigp{#1}}
\newcommand{\TODO}[1]{}
%\newcommand{\TODO}[1]{\textcolor{red}{[TODO: #1]}}
\newcommand{\code}[1]{{\tt #1}}
\newcommand{\BS}{\linebreak[0]$\backslash$\linebreak[0]}
%\newcommand{\mypreamble}[1]{
%  \rule{\textwidth}{1pt}
%  {\em #1} \\
%  \rule[6pt]{\textwidth}{1pt}
%  \medskip
%}
\newtheorem{definition}{Definition}
\newenvironment{tightitemize}{
% \vspace{-2mm}
\begin{itemize}
  \setlength{\itemsep}{0pt}
  \setlength{\parskip}{0pt}
  \setlength{\parsep}{0pt}
}{
% \vspace{-0.5mm}
\end{itemize}}
\newenvironment{tightenumerate}{
% \vspace{-2mm}
\begin{enumerate}
  \setlength{\itemsep}{0pt}
  \setlength{\parskip}{0pt}
  \setlength{\parsep}{0pt}
}{
% \vspace{-0.5mm}
\end{enumerate}}

\setcounter{secnumdepth}{3}

\begin{document}

\begin{titlepage}
\begin{center}
\textsc{\Huge Operating System Auditing and Monitoring} \\
\vspace{2cm}
\textsc{\huge Yongzheng Wu} \\
{B.Comp.(Hons.), National University of Singapore} \\
\vspace{3cm}
\includegraphics[width=0.23\textwidth]{nus-logo.png} \\
\vspace{5cm}
{\large\bf
A THESIS SUBMITTED \\
\vspace{0.5cm}
FOR THE DEGREE OF DOCTOR OF PHILOSOPHY \\
DEPARTMENT OF COMPUTER SCIENCE \\
NATIONAL UNIVERSITY OF SINGAPORE \\
\vspace{1cm}
2011}
\end{center}
\end{titlepage}

\thispagestyle{empty}

\frontmatter
\input{ackno}
\tableofcontents
\input{summary}
\listoftables
\listoffigures
% list of Illustrations
% list of Symbols

\mainmatter
\input{intro}
\input{bg}
  \input{bg-win}
  \input{bg-mon}
\chapter{Monitoring Infrastructure} \label{sec:mon}
  \input{mon-intro}
  \input{lbox/main}
  \input{winresmon/main}
\input{sensor/main}
\chapter{Visualizing System/Software Traces} \label{sec:vis}
  \input{vis-intro}
  \input{depvis/main}
  \input{lviz/main}
\chapter{Binary Integrity} \label{sec:auth}
  \input{auth-intro}
  \input{binauth/main}
  \input{binint/main}
\input{conclusion}

\backmatter
\bibliographystyle{plain}
\bibliography{thesis}

\end{document}
