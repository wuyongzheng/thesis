...
% \mypreamble{
% This section presents LBox, our user-level logging framework for Linux.
...
the system logging mechanism \code{syslog} (Section~\ref{sec:syslog}), e.g.
...
\code{proc} (Section~\ref{sec:ptrace}) do not provide these guarantees.
...
These user identities are used to identify the owner of a process in different
contexts.
For example, the {\em fsuid} is used during file system operations.
...
% is the simple monitor given in Figure~\ref{fig:lbox-eg-prog} from Section~\ref{sec:lbox-eg}.
...
For example, one can specify that all files under \code{/etc}
are included by using the SELF+SUBDIRECTORIES flag.
Alternatively, one can specify that only the \code{/etc} directory itself
...
which is setuid (setgid) to user $w$ as long as the effective user (group) id
...
It may be the case that one of the processes being monitored is
...
\begin{figure}[htb]
\centering
\subfigure[Parent-Child Process Tree]{
\begin{tikzpicture}[x=2mm,y=2mm,font=\scriptsize,>=latex]
\tikzstyle{proc}=[rectangle,draw]
\node at (27,10) [proc] (b1) {bash (14560)};
\node at (45,12) [proc] (b2) {bash (15343)};
\node at (10,23) [proc] (h1) {httpd (4010)};
\node at (5,17) [proc] (h2) {httpd (4011)};
\node at (9,13) [proc] (h3) {httpd (4012)};
\node at (15,9) [proc] (h4) {httpd (4013)};
\node at (28,27) [proc] (i1) {init (1)};
\node at (16,4) [proc] (l1) {\bf lb-mon (14940)};
\node at (30,4) [proc] (l2) {\bf lb-mon (14941)};
\node at (45,8) [proc] (l3) {\bf lb-mon (15349)};
\node at (36,22) [proc] (s1) {sshd (14012)};
\node at (45,16) [proc] (s2) {sshd:alice (15342)};
\node at (27,16) [proc] (s3) {sshd:root (14120)};
\node at (45,04) [proc] (v1) {vim (15350)};
\draw [->] (i1) -- (h1);
\draw [->] (h1) -- (h2);
\draw [->] (h1) -- (h3);
\draw [->] (h1) -- (h4);
...
\draw [->] (b1) -- (l2);
\end{tikzpicture}
\label{fig:lbox-mon-tree1}
}

\subfigure[Monitor DAG]{
\begin{tikzpicture}[x=2mm,y=2mm,font=\scriptsize,>=latex]
\tikzstyle{proc}=[rectangle,draw]
\node at (8,10) [proc] (b1) {bash (14560)};
\node at (5,13) [proc] (b2) {bash (15343)};
\node at (49,17) [proc] (h1) {httpd (4010)};
\node at (48,13) [proc] (h2) {httpd (4011)};
\node at (47,9) [proc] (h3) {httpd (4012)};
\node at (46,5) [proc] (h4) {httpd (4013)};
\node at (0,19) [proc] (i1) {init (1)};
\node at (24,23) [proc] (l1) {\bf lb-mon (14940)};
\node at (39,22) [proc] (l2) {\bf lb-mon (14941)};
\node at (31,8) [proc] (l3) {\bf lb-mon (15349)};
\node at (3,16) [proc] (s1) {sshd (14012)};
\node at (12,4) [proc] (s2) {sshd:alice (15342)};
\node at (10,7) [proc] (s3) {sshd:root (14120)};
\node at (24,4) [proc] (v1) {vim (15350)};
\draw [->] (l1) -- (i1);
\draw [->] (l1) -- (s1);
\draw [->] (l1) -- (b2);
\draw [->] (l1) -- (b1);
...
Figure~\ref{fig:lbox-mon-tree1} shows the process tree induced
by the parent-child relationship while Figure~\ref{fig:lbox-mon-tree2}
...
Sometimes we need to monitor all actions on a single process;
and other times we need to monitor a single action on all processes.
...
{\small\verb|  lbox_net_connect_ipv4(&event_spec, "137.132.0.0/255.255.0.0");|}
...
Finke~\cite{finke2002process} proposed a monitoring system, which reports when
applications did not run as they were supposed to, e.g. due to some
failure. Again, it is easy to write a monitor which can detect whether
tasks ran as scheduled.
...
We have implemented our prototype as a kernel module for Linux 2.6 kernel.
There are several ways of implementing the monitor mechanism:
\begin{enumerate}
\item {\bf {\tt ptrace()} or {\tt /proc}}
...
\item {\bf In-kernel system call interposition}
...
\item If the current file is marked SUBDIRECTORIES, report it (generate event),
done.
\item If the current file is marked IGNORE (neither SELF nor SUBDIRECTORIES),
ignore it, done.
...
are used to allow the monitor to get events
even if the buffer is not full.
In this sense, the monitor can choose
a trade-off between efficiency and timeliness.
...
the number of monitors of the process is not too large.

\subsection{Comparing to DTrace}
\label{sec:lbox-dtrace}

There are some similarities with Solaris DTrace (Section~\ref{sec:dtrace}),
...
Our prototype Linux implementation makes use of LSM~\cite{wright2002linux} and
version 2.6.10 of the kernel,
and thus is convenient to install
as it consists of loadable kernel modules.
The interface to the kernel is done through \code{ioctl} to a pseudo
file system in \code{/proc/lbox}.
...
\hline
\end{tabular}
\caption{\code{open(2)} micro-benchmark on Linux. All times are in seconds.}
\label{tab:lbox-open}
\end{table}
...
Systrace, on the other hand, does access control first can choose between a
...
\hline
\end{tabular}
\caption{\code{open(2)} micro-benchmark on Solaris 10}
% (SunOS 5.10 i386)}
\label{tab:lbox-sol}
\end{table}
...
\hline
\end{tabular}
\caption{\code{connect(2)} micro-benchmark}
...
Alcatraz which uses \code{ptrace} has rather
