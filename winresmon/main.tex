...
% \mypreamble{
% This section presents WinResMon,
% a resource usage monitoring framework for Windows.
...
addressed by the use of a package manager.
The Red Hat Package Manager (RPM)~\cite{ewing1996rpm}, for example,
records the currently installed packages and the
...
registry, making it difficult for the user to remove it completely.
Wang et al.~\cite{wang2004gatekeeper} have listed the most common entry points
for spyware to enter a Windows NT system.
The following are some of the configuration
...
system call.
This stack trace feature is used in our trace visualization (Section~\ref{sec:lviz}).
...
A fragment of an example configuration is shown in
Figure~\ref{fig:resmon-prior}. This example
...
\end{small}
\hrule
\caption{Example of Log Priorities for Trace Compaction}
\label{fig:resmon-prior}
\end{figure}
...
As discussed in Section~\ref{sec:bg-win}, this is more complex in practice.
...
WinResMon implements the system call interceptor by means of a kernel driver.
The driver captures system call requests made by a process by ``hooking'' the
native system calls \cite{ddj-nthooking}.
Table~\ref{tab:resmon-syscall} lists the
...
FileMon and RegMon tools (Section~\ref{sec:sysinternals}).
However, WinResMon's
...
Strace (Section~\ref{sec:ptrace}) is a Linux/UNIX tool used to intercept and log system
...
Systrace~\cite{provos2003improving} is a UNIX tool for sandboxing untrusted
code.
Unlike Systrace, which examines system-call sequences issued by the monitored
...
DTrace (Section~\ref{sec:dtrace}),
SystemTap (Section~\ref{sec:systemtap}) and
LBox (Section~\ref{sec:lbox}) are
